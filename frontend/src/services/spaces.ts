 ...
 */
export const joinSpaceWithInviteCode = async (inviteCode: string): Promise<Space> => {
  if (!inviteCode || inviteCode.trim() === '') {
    throw new Error('Invite code is required');
  }

  const response = await apiService.post('/api/spaces/join', {
...
/**
 * Regenerate invite code for a space
 */
export const regenerateInviteCode = async (spaceId: string): Promise<{ inviteCode: string }> => {
  if (!spaceId || spaceId.trim() === '') {
    throw new Error('Space ID is required');
  }

  const response = await apiService.post(`/api/spaces/${spaceId}/invite`, {});

  // Validate response has expected invite code structure
  if (!response || typeof response !== 'object' || !('inviteCode' in response)) {
    throw new Error('Invalid invite code data received from API');
  }

  const inviteResponse = response as { inviteCode: string };
  if (typeof inviteResponse.inviteCode !== 'string' || !inviteResponse.inviteCode) {
    throw new Error('Invalid invite code in response');
  }
...
/**
 * Alias for getSpaceMembers - Get members of a space
 */
export const getMembers = getSpaceMembers;

/**
 * Alias for regenerateInviteCode - Regenerate invite code for a space
 */
export const regenerateInvite = regenerateInviteCode;
