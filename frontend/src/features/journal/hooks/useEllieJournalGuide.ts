...
export function useEllieJournalGuide(
  ...
  const { setIsTyping: setContextIsTyping } = useEllie()
  ...
  const handleTyping = useCallback(() => {
    ...
    if (!isTyping) {
      ...
      setIsTyping(true)
      setContextIsTyping(true) // Update context for mobile OSK awareness
      setMood('zen')
    ...
    typingTimeoutRef.current = setTimeout(() => {
      setIsTyping(false)
      setContextIsTyping(false) // Restore visibility on mobile
      setMood('idle')
  ...
  }, [isTyping, mood, setMood, setContextIsTyping])
