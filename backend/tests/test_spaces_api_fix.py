...
def mock_current_user():
    ...
    return {"sub": "test-user-123", "email": "test@example.com", "username": "testuser"}
...
def test_empty_spaces_returns_proper_json(
    mock_boto3, mock_current_user, mock_dynamodb_table, client
):
    ...
    # Make request
    response = client.get("/api/users/spaces", headers={"Authorization": "Bearer fake-token"})
...
@patch("app.services.space.boto3.resource")
def test_create_space_returns_proper_json(
    mock_boto3, mock_current_user, mock_dynamodb_table, client
):
    ...
    response = client.post(
        "/api/spaces",
        json={"name": "Test Space", "description": "A test space", "isPublic": False},
...
@patch("app.services.space.boto3.resource")
def test_multiple_spaces_with_pagination(
    mock_boto3, mock_current_user, mock_dynamodb_table, client
):
    ...
    # Test with limit=2
    response = client.get(
        "/api/users/spaces?limit=2", headers={"Authorization": "Bearer fake-token"}
    )
