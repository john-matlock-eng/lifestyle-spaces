...
def test_create_space_generates_invite_code(dynamodb_table):
    ...
    space_data = SpaceCreate(
        name="Test Space", description="Test description", type="workspace", is_public=False
    )
    ...
    assert all(
        c in "0123456789ABCDEF" for c in result["invite_code"]
    ), "Invite code should be hex characters only"

    print(f"[OK] Space created with invite code: {result['invite_code']}")

    # Verify it's stored in DynamoDB
    space_id = result["id"]
    response = dynamodb_table.get_item(Key={"PK": f"SPACE#{space_id}", "SK": "METADATA"})

    assert "Item" in response, "Space should be stored in DynamoDB"
    assert (
        response["Item"]["invite_code"] == result["invite_code"]
    ), "Stored invite code should match returned invite code"
...
def test_create_space_stores_invite_mapping(dynamodb_table):
    ...
    service = SpaceService()
    space_data = SpaceCreate(
        name="Test Space 2", description="Test description", type="workspace", is_public=False
    )
...
def test_get_space_includes_invite_code_for_owner(dynamodb_table):
    ...
    service = SpaceService()
    space_data = SpaceCreate(
        name="Test Space 3", description="Test description", type="workspace", is_public=False
    )
    ...
    assert (
        retrieved_space["invite_code"] == invite_code
    ), "Retrieved invite code should match created invite code"
