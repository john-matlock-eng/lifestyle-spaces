...
class TestSpaceCreation:
    ...
    def test_create_space_validation_name_too_long(self, test_client):
        ...
        with patch("app.core.security.decode_token") as mock_decode:
            ...
            with patch("app.core.dependencies.UserProfileService") as mock_profile_service:
                ...
                response = test_client.post(
                    "/api/spaces",
                    json={"name": "a" * 101, "type": "workspace"},  # 101 characters
                    headers={"Authorization": "Bearer test-token"},
                )
    ...
    def test_create_space_without_authentication(self, test_client):
        ...
        # Act
        response = test_client.post("/api/spaces", json={"name": "Test Space", "type": "workspace"})
        ...
        with patch("app.core.security.decode_token") as mock_decode:
            ...
            with patch("app.core.dependencies.UserProfileService") as mock_profile_service:
                ...
                with patch("app.api.routes.spaces.SpaceService") as mock_service_class:
                    mock_service_instance = Mock()
                    mock_service_class.return_value = mock_service_instance
                    mock_service_instance.create_space.side_effect = Exception(
        ...
        with patch("app.core.security.decode_token") as mock_decode:
            ...
            with patch("app.core.dependencies.UserProfileService") as mock_profile_service:
                ...
                with patch("app.api.routes.spaces.SpaceService") as mock_service_class:
                    ...
                    mock_service_instance.get_space.side_effect = SpaceNotFoundError(
        ...
        with patch("app.core.security.decode_token") as mock_decode:
            ...
            with patch("app.core.dependencies.UserProfileService") as mock_profile_service:
                ...
                with patch("app.api.routes.spaces.SpaceService") as mock_service_class:
                    ...
                    mock_service_instance.get_space.side_effect = UnauthorizedError(
...
class TestListUserSpaces:
    ...
    def test_list_user_spaces_success(self, test_client):
        ...
        # Arrange
        with patch("app.core.security.decode_token") as mock_decode, patch(
            ...
            with patch("app.services.space.SpaceService.list_user_spaces") as mock_list:
                ...
                # Act
                response = test_client.get(
                    "/api/users/spaces", headers={"Authorization": "Bearer test-token"}
                )
    ...
    def test_list_user_spaces_with_search_filter(self, test_client):
        ...
        with patch("app.core.security.decode_token") as mock_decode, patch(
            ...
            with patch("app.services.space.SpaceService.list_user_spaces") as mock_list:
                ...
                # Act
                response = test_client.get(
                    "/api/users/spaces?search=alpha", headers={"Authorization": "Bearer test-token"}
                )
    ...
    def test_list_user_spaces_filter_by_role(self, test_client):
        ...
        with patch("app.core.security.decode_token") as mock_decode, patch(
            ...
            with patch("app.api.routes.users.SpaceService") as mock_service_class:
                ...
                # Act
                response = test_client.get(
                    "/api/users/spaces?role=owner", headers={"Authorization": "Bearer test-token"}
                )
    ...
    def test_get_space_members_non_member_forbidden(self, test_client):
        ...
        with patch("app.core.security.decode_token") as mock_decode, patch(
            ...
            with patch("app.api.routes.spaces.SpaceService") as mock_service_class:
                ...
                mock_service_instance.get_space_members.side_effect = UnauthorizedError(
                    "You are not a member of this space"
                )
    ...
    def test_get_members_space_not_found(self, test_client):
        ...
        with patch("app.core.security.decode_token") as mock_decode, patch(
            ...
            with patch("app.api.routes.spaces.SpaceService") as mock_service_class:
                ...
                mock_service_instance.get_space_members.side_effect = SpaceNotFoundError(
                    f"Space {space_id} not found"
                )
    ...
    def test_maximum_spaces_per_user(self, test_client):
        ...
        with patch("app.core.security.decode_token") as mock_decode:
            ...
            with patch("app.api.routes.spaces.SpaceService") as mock_service_class:
                ...
                mock_service_instance.create_space.side_effect = SpaceLimitExceededError(
                    "User has reached maximum number of spaces"
                )
...
class TestAuthenticationAndAuthorization:
    ...
    def test_expired_jwt_token(self, test_client):
        ...
        with patch("app.core.security.decode_token") as mock_decode:
            ...
            # Act
            response = test_client.get(
                "/api/users/spaces", headers={"Authorization": "Bearer expired-token"}
            )
    ...
    def test_invalid_jwt_signature(self, test_client):
        ...
        with patch("app.core.security.decode_token") as mock_decode:
            ...
            # Act
            response = test_client.get(
                "/api/users/spaces", headers={"Authorization": "Bearer invalid-signature-token"}
            )
    ...
    def test_malformed_jwt_token(self, test_client):
        ...
        # Act
        response = test_client.get(
            "/api/users/spaces", headers={"Authorization": "Bearer not.a.valid.jwt"}
        )
