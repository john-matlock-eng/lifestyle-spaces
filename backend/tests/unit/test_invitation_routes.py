...
class TestInvitationRoutes:
    ...
    def test_create_invitation_already_exists_error(self):
        ...
        with patch("app.api.routes.invitations.SpaceService") as mock_space_service, patch(
            ...
            mock_invitation_instance = Mock()
            mock_invitation_instance.create_invitation.side_effect = InvitationAlreadyExistsError(
    ...
    def test_accept_invitation_success(self):
        ...
        with patch("app.api.routes.invitations.InvitationService") as mock_service:
            ...
            response = self.client.post(
                "/api/invitations/inv123/accept", json={"invitation_code": "code123"}
            )
    ...
    def test_accept_invitation_invalid_invitation_error(self):
        ...
        with patch("app.api.routes.invitations.InvitationService") as mock_service:
            mock_service_instance = Mock()
            mock_service_instance.accept_invitation.side_effect = InvalidInvitationError(
                "Invalid invitation code"
            )
            mock_service.return_value = mock_service_instance

            response = self.client.post(
                "/api/invitations/inv123/accept", json={"invitation_code": "invalid_code"}
            )
    ...
    def test_accept_invitation_expired_error(self):
        ...
        with patch("app.api.routes.invitations.InvitationService") as mock_service:
            mock_service_instance = Mock()
            mock_service_instance.accept_invitation.side_effect = InvitationExpiredError(
