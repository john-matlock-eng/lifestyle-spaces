...
class TestUserProfileRouteErrors:
        ...
        with patch("app.api.routes.user_profile.UserProfileService") as mock_service:
            ...
            with pytest.raises(HTTPException) as exc:
                await user_profile_routes.get_user_profile(
                    current_user={"email": "test@test.com"}, user_id="user123"
                )
    ...
    async def test_get_profile_cognito_sync_with_email(self):
        ...
        with patch("app.api.routes.user_profile.UserProfileService") as mock_service, patch(
            "app.api.routes.user_profile.CognitoService"
        ) as mock_cognito:
    ...
    async def test_update_profile_too_many_requests(self):
        ...
        with patch("app.api.routes.user_profile.UserProfileService") as mock_service:
            error = ClientError({"Error": {"Code": "TooManyRequestsException"}}, "UpdateItem")
        ...
        with patch("app.api.routes.user_profile.UserProfileService") as mock_service:
            ...
            with pytest.raises(HTTPException) as exc:
                await user_profile_routes.complete_onboarding(
                    request=None, current_user={"email": "test@test.com"}, user_id="user123"
                )
            assert exc.value.status_code == 404
    ...
    async def test_complete_onboarding_send_email_success(self):
        ...
        with patch("app.api.routes.user_profile.UserProfileService") as mock_service, patch(
            "app.api.routes.user_profile.EmailService"
        ) as mock_email:
            now = datetime.now(timezone.utc).isoformat()
            mock_profile = {
                "id": "user123",
            ...
            mock_email.return_value.send_welcome_email.assert_called_once_with(
    ...
    async def test_complete_onboarding_email_failure(self):
        ...
        with patch("app.api.routes.user_profile.UserProfileService") as mock_service, patch(
            "app.api.routes.user_profile.EmailService"
        ) as mock_email, patch("app.api.routes.user_profile.logger") as mock_logger:
            now = datetime.now(timezone.utc).isoformat()
            mock_profile = {
                "id": "user123",
                "email": "test@test.com",
...
class TestSpaceServiceErrors:
    ...
    def test_list_user_spaces_skip_deleted(self):
        ...
        with patch.object(service.table, "query") as mock_query, patch.object(
            service.table, "get_item"
        ) as mock_get_item:
    ...
    def test_join_space_with_invite_code_space_id_from_item(self):
        ...
        with patch.object(service.table, "get_item") as mock_get_item, patch.object(
            service.table, "put_item"
        ) as mock_put, patch.object(service, "get_space") as mock_get:
    ...
    def test_join_space_full_flow(self):
        ...
        with patch.object(service.table, "get_item") as mock_get_item, patch.object(
            service.table, "query"
        ) as mock_query, patch.object(service.table, "put_item") as mock_put, patch.object(
            service, "get_space"
        ) as mock_get:
            # Setup mocks for full flow
            mock_get_item.side_effect = [{}, {}]  # No direct invite item  # Not already a member
            mock_query.return_value = {"Items": [{"space_id": "space123"}]}
            mock_put.return_value = {}
    ...
    async def test_lifespan_startup_shutdown(self):
        ...
        with patch("app.main.logger") as mock_logger:
            ...
            mock_logger.info.assert_any_call(
                f"Starting Lifestyle Spaces API v{__import__('app').__version__}"
            )
...
class TestAdditionalEdgeCases:
    ...
    async def test_update_profile_resource_in_use(self):
        ...
        with patch("app.api.routes.user_profile.UserProfileService") as mock_service:
            error = ClientError({"Error": {"Code": "ResourceInUseException"}}, "UpdateItem")
            mock_service.return_value.update_user_profile.side_effect = error
