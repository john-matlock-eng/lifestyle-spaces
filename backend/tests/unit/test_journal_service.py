...
class TestJournalService:
    ...
    def test_get_user_role_success(self, journal_service, mock_table):
        ...
        mock_table.get_item.return_value = {
            "Item": {"PK": "SPACE#space-123", "SK": "MEMBER#user-123", "role": "owner"}
        }
    ...
    def test_create_journal_entry_success(
        self, mock_get_space, mock_is_member, journal_service, mock_table, sample_journal_data
    ):
        """Test creating journal entry - success."""
        # Mock space exists
        mock_get_space.return_value = {"id": "space-123", "name": "Test Space"}

        # Mock user is member
        mock_is_member.return_value = True

        result = journal_service.create_journal_entry(
            space_id="space-123", user_id="user-123", data=sample_journal_data
        )
    ...
    def test_create_journal_entry_space_not_found(
        self, mock_get_space, journal_service, sample_journal_data
    ):
        ...
        with pytest.raises(SpaceNotFoundError):
            journal_service.create_journal_entry(
                space_id="space-456", user_id="user-123", data=sample_journal_data
            )

    @patch("app.services.journal.JournalService._is_space_member")
    @patch("app.services.journal.JournalService._get_space")
    def test_create_journal_entry_not_member(
        self, mock_get_space, mock_is_member, journal_service, sample_journal_data
    ):
        ...
        with pytest.raises(UnauthorizedError):
            journal_service.create_journal_entry(
                space_id="space-123", user_id="user-456", data=sample_journal_data
            )
    ...
    def test_update_journal_entry_success(self, mock_author, journal_service, mock_table):
        ...
        update_data = JournalUpdate(
            title="New Title", content="New content", tags=["new"], is_pinned=True
        )

        result = journal_service.update_journal_entry(
            "space-123", "journal-123", "user-123", update_data
        )

        assert result["title"] == "New Title"
        assert result["content"] == "New content"
        assert result["is_pinned"] is True
        # update_item is called at least once for the journal update
        # Additional calls may happen for background AI metadata generation
    ...
    def test_update_journal_entry_not_author(self, journal_service, mock_table):
        ...
        with pytest.raises(UnauthorizedError):
            journal_service.update_journal_entry(
                "space-123", "journal-123", "user-456", update_data
            )
    ...
    def test_delete_journal_entry_unauthorized(self, mock_role, journal_service, mock_table):
        ...
        mock_table.get_item.return_value = {
            "Item": {"journal_id": "journal-123", "space_id": "space-123", "user_id": "user-123"}
        }
    ...
    @patch("app.services.journal.JournalService._is_space_member")
    @patch("app.services.journal.JournalService._get_space")
    def test_list_space_journals_success(
        self, mock_get_space, mock_is_member, mock_author, journal_service, mock_table
    ):
    ...
    @patch("app.services.journal.JournalService._is_space_member")
    @patch("app.services.journal.JournalService._get_space")
    def test_list_space_journals_with_filters(
        self, mock_get_space, mock_is_member, mock_author, journal_service, mock_table
    ):
    ...
    @patch("app.services.journal.JournalService._is_space_member")
    @patch("app.services.journal.JournalService._get_space")
    def test_list_space_journals_space_not_found(
        self, mock_get_space, mock_is_member, journal_service
    ):
    ...
    @patch("app.services.journal.JournalService._is_space_member")
    @patch("app.services.journal.JournalService._get_space")
    def test_list_space_journals_unauthorized(
        self, mock_get_space, mock_is_member, journal_service
    ):
    ...
    @patch("app.services.journal.JournalService._is_space_member")
    @patch("app.services.journal.JournalService._get_space")
    def test_list_space_journals_pagination(
        self, mock_get_space, mock_is_member, mock_author, journal_service, mock_table
    ):
    ...
    @patch("app.services.journal.JournalService._get_author_info")
    @patch("app.services.journal.JournalService._is_space_member")
    def test_list_user_journals_success(
        self, mock_is_member, mock_author, journal_service, mock_table
    ):
    ...
    @patch("app.services.journal.JournalService._is_space_member")
    def test_list_user_journals_filters_inaccessible_spaces(
        self, mock_is_member, journal_service, mock_table
    ):
