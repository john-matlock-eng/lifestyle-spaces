    ...
    def test_create_test_pool_reraise_exception(self):
        ...
        try:
            with patch("app.services.cognito.boto3.client") as mock_boto:
                ...
                mock_client.create_user_pool.return_value = {"UserPool": {"Id": "created-pool-id"}}
                ...
                mock_client.list_user_pools.return_value = {
                    "UserPools": [{"Name": "different-pool", "Id": "different-id"}]
                }
    ...
    def test_get_username_by_email_returns_found_user(self):
        ...
        with patch("app.services.cognito.boto3.client") as mock_boto:
            ...
            mock_client.list_users.return_value = {
                "Users": [{"Username": "user123", "Attributes": []}]
            }
        ...
        try:
            with patch("app.services.cognito.boto3.client") as mock_boto:
                ...
                with pytest.raises(RuntimeError) as exc_info:
                    CognitoService()
                assert "Cannot auto-create Cognito pools in Lambda environment" in str(
                    exc_info.value
                )
    ...
    def test_create_test_client_raises_in_lambda_environment(self):
        ...
        try:
            with patch("app.services.cognito.boto3.client") as mock_boto:
                ...
                with pytest.raises(RuntimeError) as exc_info:
                    CognitoService()
                assert "Cannot auto-create Cognito clients in Lambda environment" in str(
                    exc_info.value
                )
