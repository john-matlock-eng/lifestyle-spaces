...
class TestSpacesRoutes:
    ...
    def test_update_space_not_found_error(self):
        ...
        with patch("app.api.routes.spaces.SpaceService") as mock_service:
            ...
            response = self.client.put("/api/spaces/space123", json={"name": "Updated Space"})
    ...
    def test_update_space_unauthorized_error(self):
        ...
        with patch("app.api.routes.spaces.SpaceService") as mock_service:
            mock_service_instance = Mock()
            mock_service_instance.update_space.side_effect = UnauthorizedError(
                "Unauthorized to update space"
            )
            mock_service.return_value = mock_service_instance
    ...
    def test_delete_space_unauthorized_error(self):
        ...
        with patch("app.api.routes.spaces.SpaceService") as mock_service:
            mock_service_instance = Mock()
            mock_service_instance.delete_space.side_effect = UnauthorizedError(
                "Unauthorized to delete space"
            )
            mock_service.return_value = mock_service_instance
