...
def test_config_model_validate():
    ...
    data = {"environment": "testing", "dynamodb_table": "test-table", "aws_region": "us-west-2"}
...
def test_user_response_default_username():
    ...
    user = UserResponse(
        id="user123", email="test@example.com", username="testuser", created_at=now, updated_at=now
    )
...
class TestSpaceServiceMissingLines:
    ...
    def test_get_space_client_error(self):
        ...
        with patch.object(service.table, "get_item") as mock_get:
            ...
            mock_get.side_effect = ClientError({"Error": {"Code": "InternalError"}}, "GetItem")
