...
class TestSpaceServiceCoverage:
    ...
    def test_create_space_with_all_fields(self):
        ...
        result = service.create_space(space=space, owner_id="user123")
    ...
    def test_update_space_not_owner_or_admin(self):
        ...
        self.table.put_item(Item={"PK": "SPACE#123", "SK": "MEMBER#user456", "role": "member"})

        service = SpaceService()
        update = SpaceUpdate(name="New Name")

        with pytest.raises(UnauthorizedError):
            service.update_space(space_id="123", update=update, user_id="user456")
    ...
    def test_delete_space_success(self):
        ...
        self.table.put_item(Item={"PK": "SPACE#123", "SK": "MEMBER#user123", "role": "owner"})
        self.table.put_item(Item={"PK": "SPACE#123", "SK": "MEMBER#user456", "role": "member"})

        # Add invite code
        self.table.put_item(Item={"PK": "INVITE#ABC123", "SK": "SPACE#123", "space_id": "123"})

        service = SpaceService()
        service.delete_space(space_id="123", user_id="user123")

        # Verify space metadata was deleted
        response = self.table.get_item(Key={"PK": "SPACE#123", "SK": "METADATA"})
    ...
    def test_list_user_spaces_with_filters(self):
        ...
        result = service.list_user_spaces(user_id="user123", is_public=True)
        assert len(result["spaces"]) == 1
        assert result["spaces"][0]["name"] == "Public Space"

        # Test filter by role
        result = service.list_user_spaces(user_id="user123", role="owner")
        assert len(result["spaces"]) == 1
    ...
    def test_list_user_spaces_with_search(self):
        ...
        # Search for "dev"
        result = service.list_user_spaces(user_id="user123", search="dev")
        assert len(result["spaces"]) == 1
    ...
    def test_join_space_already_member(self):
        ...
        self.table.put_item(Item={"PK": "SPACE#123", "SK": "MEMBER#user123", "role": "member"})

        service = SpaceService()

        with pytest.raises(AlreadyMemberError, match="already a member"):
            service.join_space_with_invite_code(invite_code="ABC123", user_id="user123")
    ...
    def test_join_space_invalid_invite_code(self):
        ...
        with pytest.raises(InvalidInviteCodeError, match="Invalid invite code"):
            service.join_space_with_invite_code(invite_code="INVALID", user_id="user123")
    ...
    def test_get_space_members_public_space(self):
        ...
        service = SpaceService()
        members = service.get_space_members(space_id="123", user_id="user456")  # Not a member
    ...
    def test_space_service_database_error_handling(self):
        ...
        with patch.object(service.table, "batch_writer") as mock_batch:
            ...
            with pytest.raises(ClientError):
                service.create_space(space=space, owner_id="user123")
