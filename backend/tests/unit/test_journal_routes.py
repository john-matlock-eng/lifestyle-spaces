...
class TestJournalRoutes:
    ...
    def setup_method(self):
        ...
        self.sample_journal_response = {
            ...
            "word_count": 2,
            "is_pinned": False,
            "author": {"user_id": "user-123", "username": "testuser", "display_name": "Test User"},
        }
    ...
    def test_create_journal_validation_error(self):
        ...
        with patch("app.api.routes.journals.JournalService") as mock_service:
            ...
            response = self.client.post(
                "/api/spaces/space-123/journals",
                json={"spaceId": "space-123", "title": "", "content": "Test content"},
            )
    ...
    def test_create_journal_space_not_found(self):
        ...
        with patch("app.api.routes.journals.JournalService") as mock_service:
            mock_service_instance = Mock()
            mock_service.return_value = mock_service_instance
            mock_service_instance.create_journal_entry.side_effect = SpaceNotFoundError(
                "Space not found"
            )

            response = self.client.post(
                "/api/spaces/space-456/journals",
                json={"spaceId": "space-456", "title": "Test Journal", "content": "Test content"},
            )
    ...
    def test_create_journal_unauthorized(self):
        ...
        with patch("app.api.routes.journals.JournalService") as mock_service:
            mock_service_instance = Mock()
            mock_service.return_value = mock_service_instance
            mock_service_instance.create_journal_entry.side_effect = UnauthorizedError(
                "Not authorized"
            )

            response = self.client.post(
                "/api/spaces/space-123/journals",
                json={"spaceId": "space-123", "title": "Test Journal", "content": "Test content"},
            )
    ...
    def test_list_space_journals_space_not_found(self):
        ...
        with patch("app.api.routes.journals.JournalService") as mock_service:
            mock_service_instance = Mock()
            mock_service.return_value = mock_service_instance
            mock_service_instance.list_space_journals.side_effect = SpaceNotFoundError(
                "Space not found"
            )
    ...
    def test_list_space_journals_unauthorized(self):
        ...
        with patch("app.api.routes.journals.JournalService") as mock_service:
            mock_service_instance = Mock()
            mock_service.return_value = mock_service_instance
            mock_service_instance.list_space_journals.side_effect = UnauthorizedError(
                "Not authorized"
            )
    ...
    def test_get_journal_not_found(self):
        ...
        with patch("app.api.routes.journals.JournalService") as mock_service:
            mock_service_instance = Mock()
            mock_service.return_value = mock_service_instance
            mock_service_instance.get_journal_entry.side_effect = JournalNotFoundError(
                "Journal not found"
            )
    ...
    def test_get_journal_unauthorized(self):
        ...
        with patch("app.api.routes.journals.JournalService") as mock_service:
            mock_service_instance = Mock()
            mock_service.return_value = mock_service_instance
            mock_service_instance.get_journal_entry.side_effect = UnauthorizedError(
                "Not authorized"
            )
    ...
    def test_update_journal_not_found(self):
        ...
        with patch("app.api.routes.journals.JournalService") as mock_service:
            mock_service_instance = Mock()
            mock_service.return_value = mock_service_instance
            mock_service_instance.update_journal_entry.side_effect = JournalNotFoundError(
                "Journal not found"
            )

            response = self.client.put(
                "/api/spaces/space-123/journals/journal-456", json={"title": "Updated Title"}
            )
    ...
    def test_update_journal_unauthorized(self):
        ...
        with patch("app.api.routes.journals.JournalService") as mock_service:
            mock_service_instance = Mock()
            mock_service.return_value = mock_service_instance
            mock_service_instance.update_journal_entry.side_effect = UnauthorizedError(
                "Not authorized"
            )

            response = self.client.put(
                "/api/spaces/space-123/journals/journal-123", json={"title": "Updated Title"}
            )
    ...
    def test_update_journal_validation_error(self):
        ...
        with patch("app.api.routes.journals.JournalService") as mock_service:
            ...
            response = self.client.put(
                "/api/spaces/space-123/journals/journal-123", json={"title": ""}
            )
    ...
    def test_delete_journal_not_found(self):
        ...
        with patch("app.api.routes.journals.JournalService") as mock_service:
            mock_service_instance = Mock()
            mock_service.return_value = mock_service_instance
            mock_service_instance.delete_journal_entry.side_effect = JournalNotFoundError(
                "Journal not found"
            )
    ...
    def test_delete_journal_unauthorized(self):
        ...
        with patch("app.api.routes.journals.JournalService") as mock_service:
            mock_service_instance = Mock()
            mock_service.return_value = mock_service_instance
            mock_service_instance.delete_journal_entry.side_effect = UnauthorizedError(
                "Not authorized"
            )
    ...
    def test_list_user_journals_with_pagination(self):
        ...
        with patch("app.api.routes.journals.JournalService") as mock_service:
            ...
            mock_service_instance.list_user_journals.assert_called_once_with(
                user_id="user-123", page=2, page_size=10
            )
