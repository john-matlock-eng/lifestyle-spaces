...
class SpaceService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Initializing SpaceService with table: {self.table_name}, region: {aws_region}"
        )
    ...
    def create_space(self, space: SpaceCreate, owner_id: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"[CREATE_SPACE] Starting space creation for owner_id={owner_id}, space_name={space.name}"
        )
        ...
        # Write all items
        logger.info(
            f"[CREATE_SPACE] Writing to DynamoDB: space_item with invite_code={invite_code}"
        )
    ...
    def get_space(self, space_id: str, user_id: str) -> Dict[str, Any]:
        ...
        if is_owner or user_role in ["owner", "admin"]:
            ...
            logger.info(
                f"Including invite_code for user {user_id} in space {space_id} (is_owner={is_owner}, role={user_role})"
            )
        else:
            logger.debug(
                f"Excluding invite_code for user {user_id} in space {space_id} (is_owner={is_owner}, role={user_role})"
            )
    ...
    def delete_space(self, space_id: str, user_id: str) -> None:
        ...
        response = self.table.query(KeyConditionExpression=Key("PK").eq(f"SPACE#{space_id}"))
    ...
    def list_user_spaces(
        ...
        logger.info(
            f"list_user_spaces called - user_id: {user_id}, page: {page}, page_size: {page_size}"
        )
        ...
        if not user_id:
            ...
            return {"spaces": [], "total": 0, "page": page, "page_size": page_size}
    ...
    def remove_member(self, space_id: str, member_id: str, removed_by: str) -> None:
        ...
        if not self.can_edit_space(space_id, removed_by):
            raise UnauthorizedError(
                f"User {removed_by} cannot remove members from space {space_id}"
            )
        ...
        self.table.delete_item(Key={"PK": f"SPACE#{space_id}", "SK": f"MEMBER#{member_id}"})
    ...
    def join_space_with_invite_code(self, invite_code: str, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"space_id": space_id, "name": space["name"], "role": "member", "joined_at": now}
    ...
    def regenerate_invite_code(self, space_id: str) -> str:
        ...
        if old_code:
            try:
                self.table.delete_item(Key={"PK": f"INVITE#{old_code}", "SK": f"SPACE#{space_id}"})
            except Exception:
