...
class UserProfileService:
    ...
    def get_or_create_user_profile(
        self, user_id: str, cognito_attributes: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if existing_profile:
            ...
            self.db.update_item(
                f"USER#{user_id}", "PROFILE", {"last_seen": datetime.now(timezone.utc).isoformat()}
            )
            ...
            if not existing_profile.get("display_name"):
                updates["display_name"] = cognito_attributes.get(
        ...
        profile_data = {
            ...
            "display_name": cognito_attributes.get("display_name")
            or cognito_attributes.get("username")
            or f"User {user_id[:8]}",
    ...
    def get_batch_user_profiles(self, user_ids: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        for user_id in user_ids:
            keys.append({"PK": f"USER#{user_id}", "SK": "PROFILE"})
