...
class CognitoService:
    ...
    def _create_test_client(self) -> str:
        ...
        try:
            ...
            clients = self.client.list_user_pool_clients(
                UserPoolId=self.user_pool_id, MaxResults=10
            )
    ...
    def sign_up(self, user: UserCreate) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"user_sub": user_sub, "username": user.username, "email": user.email}
    ...
    def confirm_user(self, email: str) -> None:
        ...
        try:
            self.client.admin_confirm_sign_up(
                UserPoolId=self.user_pool_id, Username=email  # Using email as username
            )
        except ClientError:
    ...
    def get_user(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not user_info["preferred_username"]:
                user_info["preferred_username"] = (
                    (user_info["email"].split("@")[0] if user_info["email"] else "")
                    or (f"user_{user_info['id'][:8]}" if user_info["id"] else "")
                    or "unknown"
                )
        ...
        if attributes:
            try:
                self.client.update_user_attributes(
                    AccessToken=access_token, UserAttributes=attributes
                )
            except ClientError as e:
                if e.response["Error"]["Code"] == "NotAuthorizedException":
                    raise InvalidCredentialsError("Invalid access token")
