...
class DynamoDBClient:
    ...
    def query(
        self, pk: str, sk_prefix: Optional[str] = None, index_name: Optional[str] = None
    ) -> list:
        """
        ...
        kwargs = {"KeyConditionExpression": Key(pk_key).eq(pk)}

        if sk_prefix:
            kwargs["KeyConditionExpression"] = kwargs["KeyConditionExpression"] & Key(
                sk_key
    ...
    def update_item(
        self, pk: str, sk: str, updates: dict, return_values: str = "ALL_NEW"
    ) -> Optional[dict]:
        """
