...
async def create_space(space: SpaceCreate, current_user: dict = Depends(get_current_user)):
    ...
    try:
        logger.info(
            f"[API_CREATE_SPACE] Request received from user={current_user.get('sub')}, space_name={space.name}"
        )

        service = SpaceService()
        result = service.create_space(space=space, owner_id=current_user.get("sub", ""))

        logger.info(
            f"[API_CREATE_SPACE] Service returned result with invite_code={result.get('invite_code')}"
        )
        ...
        logger.info(
            f"[API_CREATE_SPACE] Returning response with invite_code={response.invite_code}"
        )
        return response
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to create space"
        )
...
async def get_space(space_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        service = SpaceService()
        result = service.get_space(space_id=space_id, user_id=current_user.get("sub", ""))
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get space"
        )


@router.put("/{space_id}", response_model=SpaceResponse)
async def update_space(
    space_id: str, update: SpaceUpdate, current_user: dict = Depends(get_current_user)
):
    """Update space (owner/admin only)."""
    try:
        service = SpaceService()
        result = service.update_space(
            space_id=space_id, update=update, user_id=current_user.get("sub", "")
        )

        # Get full space info for response
        full_space = service.get_space(space_id=space_id, user_id=current_user.get("sub", ""))
        ...
        return SpaceResponse(
            ...
            calendar_url=full_space.get("calendar_url"),
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update space"
        )
...
async def delete_space(space_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        service = SpaceService()
        service.delete_space(space_id=space_id, user_id=current_user.get("sub", ""))

        return SuccessResponse(message=f"Space {space_id} deleted successfully")
    except SpaceNotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    except UnauthorizedError as e:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(e))
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to delete space"
        )


@router.get("/{space_id}/members", response_model=MembersListResponse)
async def get_space_members(space_id: str, current_user: dict = Depends(get_current_user)):
    """Get members (members only or public)."""
    try:
        logger.info(f"Getting members for space {space_id} by user {current_user.get('sub')}")
        service = SpaceService()
        members = service.get_space_members(space_id=space_id, user_id=current_user.get("sub", ""))
        ...
        return MembersListResponse(
            members=formatted_members, total=len(formatted_members), has_more=False
        )
    except SpaceNotFoundError as e:
...
async def regenerate_invite_code(space_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        return {"invite_code": new_code, "invite_url": f"/join/{new_code}"}
...
async def join_space_with_code(
    invite_code: str = Body(..., embed=True), current_user: dict = Depends(get_current_user)
):
    """Join a space using an invite code."""
    try:
        service = SpaceService()
        result = service.join_space_with_invite_code(
            invite_code=invite_code, user_id=current_user.get("sub")
        )

        # Get full space details
        space = service.get_space(space_id=result["space_id"], user_id=current_user.get("sub"))
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to join space"
        )
