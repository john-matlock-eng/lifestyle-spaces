...
def list_pending_invitations(
    ...
    return {
        "invitations": [
            inv.model_dump(by_alias=True) if hasattr(inv, "model_dump") else inv
...
def validate_invite_code(code: str, space_service: SpaceService = Depends(get_space_service)):
    ...
    try:
        ...
        response = table.query(KeyConditionExpression=Key("PK").eq(f"INVITE#{code}"))
        ...
        # Get space details
        space_response = table.get_item(Key={"PK": f"SPACE#{space_id}", "SK": "METADATA"})

        if "Item" not in space_response:
            logger.warning(f"[VALIDATE_CODE] Space not found for code: {code}")
            return {"valid": False, "error": "Space no longer exists"}

        space = space_response["Item"]
        logger.info(f"[VALIDATE_CODE] Valid code for space: {space.get('name')}")

        return {"valid": True, "spaceId": space_id, "spaceName": space.get("name"), "code": code}
...
def create_invitation(
    ...
    try:
        ...
        # Create invitation
        invitation_data = InvitationCreate(space_id=space_id, invitee_email=request.email)
        ...
        if hasattr(invitation, "invitation_id"):
            ...
            return {
                ...
                "inviter_name": current_user.get(
                    "full_name", current_user.get("username", "Unknown User")
                ),
...
def create_bulk_invitations(
    ...
    try:
        ...
        if not request.emails or len(request.emails) == 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Email list cannot be empty"
            )
        ...
        for email in request.emails:
            try:
                ...
                invitation_data = InvitationCreate(
                    space_id=space_id, invitee_email=email.strip().lower()
                )

                invitation = invitation_service.create_invitation(
                    invitation_data, current_user["sub"]
                )
                ...
                failed.append({"email": email, "error": f"Failed to create invitation: {str(e)}"})
...
def get_invitations(
    ...
    try:
        ...
        logger.error(f"Traceback: {traceback.format_exc()}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get invitations"
        )
...
def accept_invitation(
    ...
    try:
        ...
        return {
            ...
            "data": {"space_name": space_name, "role": "member", "space_id": space_id},
        }
