...
class Invitation(BaseModel):
    ...
    status: InvitationStatus = Field(
        InvitationStatus.PENDING, description="Current status of the invitation"
    )
    created_at: datetime = Field(
        default_factory=datetime.utcnow, description="Timestamp of when the invitation was created"
    )
    expires_at: Optional[datetime] = Field(
        None, description="Timestamp of when the invitation expires"
    )
...
class InvitationUpdate(BaseModel):
    ...
    expires_at: Optional[datetime] = Field(
        None, description="New expiration timestamp for the invitation"
    )
